...
import setConcierge from '../actions/setConcierge'
 class Hearse extends React.Component {
    state={
        name:"Hearse",
        startTime:'7:00' ,
        closeTime:'18:00',
    ...
    validateBooking=(val)=>{
        ...
        if( date.getHours()>0 && date.getHours()<24){
            ...
            this.setState({total:this.state.charge*val },()=>{
                this.props.setConcierge(this.state.name,this.state.total)
             })
        }else{
       //  var valStore=24-val;
...
const mapDispatchToProps=(dispatch)=>({
    setConcierge(name,price){
        dispatch(setConcierge(name,price))
    }

})
export default connect(null,mapDispatchToProps)(Hearse)